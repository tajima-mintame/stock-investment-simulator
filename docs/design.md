...
| 米国株API | Finnhub | 無料枠が最も充実（60 req/min） |
| バックグラウンド処理 | APScheduler AsyncIOScheduler | FastAPIプロセス内で動作。外部ワーカー不要 |
| HTTPクライアント | httpx（async） | Finnhub呼び出し用。J-Quantsはsync→asyncio.to_thread() |
...
| カラム | 型 | 制約 | 説明 |
|--------|-----|------|------|
| symbol | TEXT | PK | 銘柄コード（例: "7203", "AAPL"） |
| market | TEXT | PK | 市場区分（"JP" or "US"） |
| name | TEXT | | 企業名 |
| sector | TEXT | | セクター |
...
全プロバイダーはこのインターフェースに準拠する。ルーターは市場区分（"JP" / "US"）をキーにプロバイダーを切り替える。
...
| 平日 22:00 JST | 米国株（全登録銘柄） | NYSE/NASDAQ 閉場後（ET→JST変換） |
